//图的邻接表存储表示,深度优先搜索遍历(Depth First Search)
#include<stdio.h>
#include<stdlib.h>
#define MAX_VERTEX_NUM 20
typedef char VerTexType;
int visited[MAX_VERTEX_NUM];  //设置访问数组，1表示已访问过，0表示未被访问
...
//邻接表的深度优先搜索算法：递归，系统栈
void DFS_adjlist(ALGraph *G, int i)
{
    ArcNode *arclist;   //边表结点指针
    //访问第一个顶点
    visited[i] = 1;     //标记已访问，i为结点位置
    //输出
    printf("%c -> ",G->vertices[i].data);
    arclist = G->vertices[i].firstarc;
    //访问下个顶点
    while(arclist){
        if(visited[arclist->adjvex] == 0){
            DFS_adjlist(G,arclist->adjvex);
        }
        arclist = arclist->nextarc;
    }
}
